...
import {
  apiDataToTimeseriesRecords,
  getMostRelevantPoolBySymbol,
  getAllTimeseriesDataOfProtocol,
} from "../utils/coverApiDataProc";
...
import { getImageSrcOfProtocol } from "../utils/protocolHelper";
...
const Cover: FC<PropsProtocol> = (props) => {
  ...
  const [
    tokens,
    setTokensInWalletsAndPools,
  ] = useState<tokensInWalletsAndPools>();
  ...
  const ListChartTimes = (props: any) => {
    const times = chartTimes.map((chartTime) => (
      <Button
        key={chartTime}
        variant={chartTimeSelected === chartTime ? "contained" : "outlined"}
        color={props.color}
        size="small"
    ...
    return (
      <Grid item xs={7} justify="flex-end" container>
        {times}
      </Grid>
    );
  };
    ...
    if (timeseriesData) {
      switch (chartType) {
        case chartTypes[0]:
          if (isMigrated === true) {
            return (
              <ProtocolPriceChartCombined
                data={timeseriesData}
                textColor={theme.palette.text.primary}
                theme={theme}
                chartTime={chartTimeSelected || chartTimes[3]}
              />
            );
          } else {
            return (
              <ProtocolPriceChart
                ...
                lineDataKey={`${type.toLowerCase()}.price`}
        ...
        case chartTypes[1]:
          return (
            <ProtocolVolumeChart
              ...
              data={timeseriesData}
              xAxisDataKey="timestamp"
              barDataKey={`${type.toLowerCase()}.swapVol`}
              barLabel={`Volume [USD]`}
              filtering={false}
              fillColorBrush={"#323342"}
            />
        ...
        case chartTypes[2]:
          return (
            <ProtocolLiquidityChart
              ...
              data={timeseriesData}
              xAxisDataKey="timestamp"
              areaDataKey={`${type.toLowerCase()}.liquidity`}
            ...
            <Paper className={classes.paper} style={{ marginTop: "10px" }}>
              <Grid container justify="space-between" alignContent="center">
                ...
                <p className={classes.infoCard}>
                  {formatCurrency(getNewestRecord(records)[type].swapVolCum)}
                </p>
              </Grid>
            </Paper>
          </Grid>
          ...
          <Grid item xs={12} sm={6}>
            <Paper className={classes.paper} style={{ marginTop: "10px" }}>
              <Grid container justify="space-between" alignContent="center">
                ...
                {tokens ? (
                  <p className={classes.infoCard}>
                    {formatToInteger(tokens[type].wallet)}
                  </p>
                ) : (
          ...
          <Grid item xs={12} sm={6}>
            <Paper className={classes.paper} style={{ marginTop: "10px" }}>
              <Grid container justify="space-between" alignContent="center">
                ...
                {tokens ? (
                  <p className={classes.infoCard}>
                    {formatToInteger(tokens[type].pool)}
                  </p>
                ) : (
                  <LinearProgress color="primary" />
                )}
  ...
  useEffect(() => {
    const fetchTimeseriesData = async () => {
      let items = await getAllTimeseriesDataOfProtocol(
        props.match.params.cover.toLowerCase()
      );
      console.log(items);
    ...
    fetch(api.cover_api.base_url)
      ...
      .then((data) => {
        let filteredProtocols: Protocol[] = data.protocols.filter(
          (p: Protocol) => p.protocolActive === true
        );
        let selectedProtocol = filteredProtocols.find(
          (p) =>
        ...
        let [poolIdNoClaim, noClaimTokenAddr] = getMostRelevantPoolBySymbol(
          selectedProtocol.protocolName,
          false,
          data.poolData
        );
        ...
        let graphRequests = pools.map((poolId) =>
          fetch(api.the_graph_api.base_url, {
            method: "POST",
            mode: "cors",
            headers: {
        ...
        Promise.all(graphRequests).then((responses) => {
          Promise.all(responses.map((r) => r.json())).then((allGraphData) => {
            let balanceClaim = 0;
            let balanceNoClaim = 0;
            for (let graphData of allGraphData) {
              for (let token of graphData.data.pool.tokens) {
                if (
                  token.symbol.indexOf(
                    `_${props.match.params.cover.toUpperCase()}_`
                  ) > -1
                ) {
            ...
            if (selectedProtocol === undefined) return;
            let coverObject = findMostRecentCoverObject(
              selectedProtocol.coverObjects
            );
              ...
              <Paper className={classes.paper}>
                ...
                >
                  <Avatar
                    ...
                    alt={`${props.match.params.cover} Token`}
                ...
                <Grid container justify="center">
                  <Grid item>
                    <Link
                      color="inherit"
                      href={
                        api.balancer_pools.base_url +
                        getNewestRecord(timeseriesData).noclaim.poolId
                      }
                    >
                  ...
                  <Grid container justify="center">
                    <Grid item>
                      <Link
                        ...
                        href={
                          api.balancer_pools.base_url +
                          getNewestRecord(timeseriesData).claim.poolId
                        }
                      >
                        <Typography variant="h5" gutterBottom>
                          CLAIM Token

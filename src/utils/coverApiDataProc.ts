...
export const getMostRelevantPoolBySymbol = (
  symbolName: string,
  claim: boolean,
  pools: any
) => {
  ...
  for (let pool in pools) {
    if (
      pools[pool].poolId.liquidity >= liqMax &&
      pools[pool].symbol.indexOf(symbolIdentifier) > -1 &&
      pools[pool].symbol.indexOf(secondIdentifier) > -1
    ) {
      poolId = pool;
...
export const apiDataToTimeseriesRecords = (data: any[]) => {
  ...
  for (let record of data) {
    ...
    if (record.protocolData.poolData) {
      if (
        record.protocolData.poolData[poolIDClaim] &&
        record.protocolData.poolData[poolIDNoClaim]
      ) {
        records.push({
          ...
          claim: {
            ...
            swapVol:
              record.protocolData.poolData[poolIDClaim].totalSwapFee -
              lastClaimVol,
        ...
        record.protocolData.poolData[poolIDClaim] === undefined &&
        record.protocolData.poolData[poolIDNoClaim]
      ) {
        records.push({
          timestamp: record.timestamp,
...
export const getAllTimeseriesDataOfProtocol = async (protocol: string) => {
  ...
  while (currentTime - startTimestamp > 1000 * 60 * 60) {
    const response = await fetch(
      `${api.cover_api.timeseries_endpoint}?startTimestamp=${startTimestamp}&endTimestamp=${currentTime}&protocol=${protocol}`
    );
...
export const filterVolumeRecords = (
  ...
  for (let record of records) {
    if (
      record.claim.swapVol != 0 ||
      record.noclaim.swapVol != 0 ||
      Math.abs(lastTimestamp - record.timestamp) >= ms
    ) {
...
export const getRecordsNotOlderThan = (
  records: TimeseriesRecord[],
  timeInMS: number
) => {
 ...
 */
export const findAllRecordsAndDistinctTimestamps = (
  allTimeseriesData: any[][]
): [number[], TimeseriesRecord[][]] => {
 ...
 */
export const setCSVsForAnyTimestamp = (
  records: TimeseriesRecord[],
  timestamps: number[],
  collateralStakedValues: number[]
) => {
  ...
  for (let i = 0; i < timestamps.length; i++) {
    ...
    if (
      records[indexLastRecordUsed] === undefined ||
      records[indexLastRecordUsed].timestamp > timestamp
    ) {
      collateralStakedValues[i] += lastCSVSeen;
    } else {
      collateralStakedValues[i] +=
        records[indexLastRecordUsed].collateralStakedValue;
...
export const isProtocolActive = (protocol: Protocol, currentTime: number) => {
  let nonExpiredTimestamps = protocol.expirationTimestamps.filter(
    (timestamp: number) => timestamp > currentTime
  );
  return nonExpiredTimestamps.length > 0 && protocol.protocolTokenAddress != "";
};
